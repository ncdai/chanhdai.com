...
export function SiteFooterBrand() {
  const gradientX1Raw = useMotionValue(705)
  ...
  const handleMouseMove = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    const viewBoxWidth = 1410

    const normalizedX = (mouseX / containerWidth) * viewBoxWidth
    const clampedX = Math.max(0, Math.min(viewBoxWidth, normalizedX))

    gradientX1Raw.set(clampedX)
  }

  const handleMouseLeave = () => {
    gradientX1Raw.set(705)
  }

  return (
    <div
      className="-translate-y-px overflow-hidden"
